...
window.shareOnWhatsapp = async function () {
    ...
    // messsage
    shareWindow.document.write('<html><body style="font-family:sans-serif;text-align:center;padding-top:20%;background-color:#f0f2f5;"><h2>Creating Link...</h2><p>Please wait while we prepare your WhatsApp message.</p></body></html>');
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            // Construct the public URL for view.php
            const baseUrl = window.location.href.substring(0, window.location.href.lastIndexOf('/'));
            const shareLink = `${baseUrl}/view.php?id=${filename}`;
            const name = document.getElementById('userName').value.trim();
            const templateTitle = activeTemplate.template_name || 'Holi';

            const message = `${name} ने आपको ${templateTitle} का सन्देश भेजा है. देखने के लिए क्लिक करें 👉 ${shareLink}`;
            ...
            shareWindow.close();
            alert('Failed to prepare sharing link.');
        }
    } catch (e) {
        console.error(e);
        shareWindow.close();
